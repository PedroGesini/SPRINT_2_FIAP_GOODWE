...
function gerarConsumo(base, variaÃ§Ã£o) {
  return MESES.map((m, i) => ({
    mes: m,
    kwh: Math.max(8, Math.round(base + Math.sin(i * 1.3) * variaÃ§Ã£o + (Math.random() - 0.5) * 6)),
  }));
}
...
const MORADORES_MOCK = [
  { nome: "Fernanda Alves", unidade: "Bloco A Â· Apto 302", cpf: "123.456.789-00", email: "fernanda.alves@email.com" },
  { nome: "Ricardo Nogueira", unidade: "Bloco B Â· Apto 118", cpf: "234.567.891-02", email: "ricardo.nog@email.com" },
  { nome: "Juliana Prado", unidade: "Bloco A Â· Apto 405", cpf: "345.678.912-03", email: "juliana.prado@email.com" },
  { nome: "Marcos VinÃ­cius Lima", unidade: "Bloco C Â· Apto 201", cpf: "456.789.123-04", email: "marcos.lima@email.com" },
  { nome: "Beatriz Coutinho", unidade: "Bloco B Â· Apto 512", cpf: "567.891.234-05", email: "bia.coutinho@email.com" },
  { nome: "AndrÃ© Salgado", unidade: "Bloco C Â· Apto 309", cpf: "678.912.345-06", email: "andre.salgado@email.com" },
].map((m, i) => ({
  ...m,
...
/* ---------------------------------------------------------------------- */
/* Conta Ãºnica do sÃ­ndico â€” provisionada uma vez na instalaÃ§Ã£o, sem       */
/* autocadastro. Em produÃ§Ã£o isso vive no banco (hash de senha), nunca    */
/* hardcoded no front-end como aqui neste protÃ³tipo.                      */
/* ---------------------------------------------------------------------- */
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-2xl p-8" style={{ background: C.surface, border: `1px solid ${C.line}` }}>
          ...
          <p className="text-sm mb-6" style={{ color: C.textMuted, fontFamily: "Inter" }}>
            Conta Ãºnica de administraÃ§Ã£o, criada uma vez na implantaÃ§Ã£o do condomÃ­nio.
          </p>
          ...
          <div className="flex flex-col gap-4">
            ...
            <Field label="Senha" type="password" value={senha} onChange={(e) => setSenha(e.target.value)} placeholder="â€¢â€¢â€¢â€¢â€¢â€¢â€¢â€¢" />
          </div>
      ...
      <div className="w-full max-w-md">
        ...
        <div className="rounded-2xl p-8" style={{ background: C.surface, border: `1px solid ${C.line}` }}>
          ...
          <p className="text-sm mb-7" style={{ color: C.textMuted, fontFamily: "Inter" }}>
            Acesse com sua conta Google para acompanhar consumo e faturas, ou entre como sÃ­ndico para gerenciar o condomÃ­nio.
          </p>
          ...
          >
            <ShieldCheck size={16} />
            Entrar como sÃ­ndico
          </button>
        </div>

        <p className="text-center text-xs mt-6" style={{ color: C.textFaint, fontFamily: "Inter" }}>
          ProtÃ³tipo acadÃªmico â€” Enterprise Challenge 2026 Â· Grupo 14
        </p>
      </div>
...
function CadastroScreen({ conta, onConcluir, onVoltar }) {
  ...
  const submeter = () => {
    if (!form.nome || form.cpf.length < 14 || !form.endereco || !form.email) {
      setErro("Preencha todos os campos â€” o CPF deve estar completo.");
      return;
    }
    setErro("");
    ...
    <div className="min-h-screen w-full flex items-center justify-center px-4" style={{ background: C.bg }}>
      <div className="w-full max-w-md">
        ...
        <div className="rounded-2xl p-8" style={{ background: C.surface, border: `1px solid ${C.line}` }}>
          ...
          <p className="text-sm mb-6" style={{ color: C.textMuted, fontFamily: "Inter" }}>
            Esses dados identificam sua unidade no carregador compartilhado do condomÃ­nio.
          </p>
          ...
          <div className="flex flex-col gap-4">
            ...
            <Field label="EndereÃ§o (bloco / apto)" value={form.endereco} onChange={set("endereco")} placeholder="Ex: Bloco A Â· Apto 304" />
    ...
    <div className="min-h-screen w-full flex items-center justify-center px-4" style={{ background: C.bg }}>
      <div className="w-full max-w-sm text-center">
        <div className="rounded-2xl p-8" style={{ background: `linear-gradient(160deg, ${C.surfaceHi}, ${C.surface})`, border: `1px solid ${C.line}` }}>
          ...
          <p className="text-sm mb-1" style={{ color: C.textMuted, fontFamily: "Inter" }}>Cadastro concluÃ­do, {nome.split(" ")[0]}</p>
          <p className="text-xs uppercase tracking-wider mb-2" style={{ color: C.textFaint }}>Seu ID de usuÃ¡rio</p>
          ...
          <p className="text-xs mt-4" style={{ color: C.textFaint, fontFamily: "Inter" }}>
            Use este ID para identificar sua sessÃ£o no carregador do condomÃ­nio.
          </p>
        </div>
      </div>
    </div>
    ...
    <div className="min-h-screen w-full flex" style={{ background: C.bg }}>
      <aside className="w-60 shrink-0 flex flex-col p-5 hidden md:flex" style={{ borderRight: `1px solid ${C.line}` }}>
        ...
        <nav className="flex flex-col gap-1 text-sm" style={{ fontFamily: "Inter" }}>
          <NavItem icon={Home} label={role === "sindico" ? "VisÃ£o geral" : "Meu consumo"} active />
          {role === "sindico" ? (
            <>
        ...
        <div className="mt-auto pt-6" style={{ borderTop: `1px solid ${C.line}` }}>
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="min-w-0">
              <p className="text-sm font-medium truncate" style={{ color: C.text }}>{activeUser?.nome || "SÃ­ndico"}</p>
              <p className="text-xs truncate" style={{ color: C.textFaint, fontFamily: "JetBrains Mono" }}>{activeUser?.id || "AdministraÃ§Ã£o"}</p>
            </div>
          </div>
  ...
  return (
    <div className="max-w-5xl">
      <header className="mb-8">
        ...
        <h1 className="text-2xl font-bold" style={{ fontFamily: "Space Grotesk", color: C.text }}>{user.nome.split(" ")[0]}, aqui estÃ¡ seu consumo</h1>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-6">
        <div className="rounded-2xl p-6 flex items-center gap-6 lg:col-span-1" style={{ background: C.surface, border: `1px solid ${C.line}` }}>
          ...
          <div>
            <p className="text-xs" style={{ color: C.textFaint }}>Consumo do mÃªs</p>
        ...
        <div className="lg:col-span-2 grid grid-cols-2 gap-5">
          <StatCard icon={Zap} label="Gasto este mÃªs" value={`R$ ${gastoMes.toFixed(2)}`} hint="referente Ã  recarga do veÃ­culo" accent={C.blue} />
      ...
      <div className="rounded-2xl p-6 mb-6" style={{ background: C.surface, border: `1px solid ${C.line}` }}>
        <h2 className="text-sm font-semibold mb-4" style={{ fontFamily: "Space Grotesk", color: C.text }}>Consumo â€” Ãºltimos 6 meses</h2>
          ...
          {faturas.map((f) => (
            <div key={f.id} className="flex items-center justify-between px-4 py-3 rounded-xl" style={{ background: C.bgSoft }}>
              <div>
                ...
                <p className="text-xs" style={{ color: C.textFaint }}>Vencimento {f.vencimento} Â· {f.id}</p>
              </div>
...
function SindicoDashboard({ moradores, setMoradores, notify }) {
  ...
  const importarExcel = useCallback((file) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (!rows.length) {
          notify("A planilha nÃ£o tem linhas para importar.");
          return;
        }
        setMoradores((prev) =>
          prev.map((m) => {
        ...
        notify(`Planilha importada â€” ${rows.length} linha(s) processada(s) e consumo atualizado.`);
      } catch {
        notify("NÃ£o foi possÃ­vel ler essa planilha. Verifique o formato (.xlsx ou .csv).");
      }
    };
  ...
  const dispararEnvio = (canal) => {
    ...
    setTimeout(() => {
      ...
      notify(
        canal === "email"
          ? `CobranÃ§a por e-mail enviada para ${alvo} morador(es) com fatura pendente.`
  ...
  return (
    <div className="max-w-6xl">
      <header className="mb-8">
        <p className="text-xs uppercase tracking-wider mb-1" style={{ color: C.textFaint, fontFamily: "Inter" }}>Painel do sÃ­ndico</p>
        <h1 className="text-2xl font-bold" style={{ fontFamily: "Space Grotesk", color: C.text }}>VisÃ£o geral do condomÃ­nio</h1>
      </header>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 mb-6">
        <StatCard icon={Users} label="Moradores cadastrados" value={moradores.length} accent={C.blue} />
        <StatCard icon={Zap} label="Consumo total (mÃªs)" value={`${totalConsumo} kWh`} accent={C.green} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-6">
        <div className="lg:col-span-2 rounded-2xl p-6" style={{ background: C.surface, border: `1px solid ${C.line}` }}>
          <h2 className="text-sm font-semibold mb-4" style={{ fontFamily: "Space Grotesk", color: C.text }}>Consumo por morador â€” mÃªs atual</h2>
        ...
        <div className="rounded-2xl p-6 flex flex-col" style={{ background: C.surface, border: `1px solid ${C.line}` }}>
          ...
          <p className="text-xs mb-4" style={{ color: C.textFaint, fontFamily: "Inter" }}>
            Planilha .xlsx/.csv com colunas Nome ou CPF e Consumo (kWh) atualiza o consumo do mÃªs.
          </p>
          <input
            ref={fileRef}
            type="file"
      ...
      <div className="rounded-2xl p-6 mb-6" style={{ background: C.surface, border: `1px solid ${C.line}` }}>
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
          <h2 className="text-sm font-semibold" style={{ fontFamily: "Space Grotesk", color: C.text }}>CobranÃ§a automÃ¡tica</h2>
        ...
        <p className="text-xs" style={{ color: C.textFaint, fontFamily: "Inter" }}>
          Envia cobranÃ§a para {inadimplentes.length || moradores.length} morador(es) com fatura pendente ou vencida. Neste protÃ³tipo o envio Ã© simulado â€” em produÃ§Ã£o conectaria a um provedor de e-mail (ex: SendGrid) e Ã  API do WhatsApp Business.
        </p>
      </div>
          ...
          <table className="w-full text-sm" style={{ fontFamily: "Inter" }}>
            <thead>
              <tr className="text-left" style={{ color: C.textFaint }}>
                ...
                <th className="pb-3 font-medium">Consumo (mÃªs)</th>
                <th className="pb-3 font-medium">SituaÃ§Ã£o</th>
              </tr>
            </thead>
            <tbody>
              {filtrados.map((m) => {
